...
class PermissionController extends Controller
{
        ...
        if (Request()->ajax()) {
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="'.route('permission.edit', $row->id).'" title="Edit" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"><span class="svg-icon svg-icon-3">
    ...
    public function edit($id){
        ...
        $data2 = Role::all();
        return view('admin.permission.edit', compact('data', 'data2'));
    }
    ...
    public function update(Request $request, $id){
        ...
        $data->name = $request->nama_permission;
        $data->save();
    ...
    public function hapusRole($id,Role $role){

        $data = Permission::findorfail($id);

        $permission =  DB::table('role_has_permissions')->count();
